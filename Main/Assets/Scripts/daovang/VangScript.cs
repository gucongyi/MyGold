...
public class VangScript : MonoBehaviour {
	...
	void FixedUpdate() {
		moveFllowTarget(GameObject.Find("Hook").transform);
	...
	void OnTriggerEnter2D(Collider2D other) {
            ...
            && GameObject.Find("HookController").GetComponent<DayCauScript>().typeAction != TypeAction.KeoCau)
        {
			...
			GameObject.Find("HookController").GetComponent<DayCauScript>().typeAction = TypeAction.KeoCau;
			GameObject.Find("Hook").GetComponent<LuoiCauScript>().velocity = -GameObject.Find("Hook").GetComponent<LuoiCauScript>().velocity;
        ...
        else if (other.tag == "fire")
        {
            ...
            GameObject GoldFire = (GameObject)Instantiate(Resources.Load("GoldFire"), position, Quaternion.identity);
            GoldFire.transform.localScale = GameObject.Find("Map").transform.localScale;
            Destroy(gameObject);
        }
	}
	...
	void moveFllowTarget(Transform target) {
		if(isMoveFollow) 
		{
            ...
            transform.position = new Vector3(target.position.x,
                                                ...
                                                transform.position.z);
            if (GameObject.Find("HookController").GetComponent<DayCauScript>().typeAction == TypeAction.Nghi) {
                //play sound
